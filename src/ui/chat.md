...
### `ChatMediaCache`
- **Does**: Caches local image textures by path and holds lightweight audio playback state for in-chat media controls (`Play`/`Stop`) with one-at-a-time playback semantics.
